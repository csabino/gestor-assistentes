...
<body class="bg-gray-50 font-sans text-gray-900 min-h-screen flex" 
    x-data="{ 
...
>
    ...
    <!-- SIDEBAR LATERAL RETRÁTIL COM TOOLTIPS E ÍCONE CENTRALIZADO -->
    <aside :class="sidebarOpen ? 'w-64' : 'w-20'" class="bg-indigo-700 text-white min-h-screen transition-all duration-300 flex flex-col justify-between shrink-0 shadow-lg relative z-50">
        <div>
            <!-- MARCA E BOTÃO RETRÁTIL -->
            <div class="h-16 flex items-center border-b border-indigo-600/80" :class="sidebarOpen ? 'px-4 justify-between' : 'px-0 justify-center'">
                ...
                <button type="button" @click="sidebarOpen = !sidebarOpen; localStorage.setItem('sidebar_open', sidebarOpen)" class="p-2 rounded-lg hover:bg-indigo-600 text-indigo-200 hover:text-white transition cursor-pointer flex items-center justify-center" title="Alternar menu">
                    <svg class="w-6 h-6 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" /></svg>
                </button>
            </div>

            <!-- NAVEGAÇÃO COM TOOLTIPS EM HOVER -->
            <nav class="p-3 space-y-2 font-medium text-sm">
                <!-- ASSISTENTES IA -->
                <div class="relative group">
                    <a href="/" class="flex items-center gap-3.5 px-3 py-2.5 rounded-xl transition font-semibold {{ $currentView === 'robots' ? 'bg-indigo-900/90 text-white shadow-sm border border-indigo-500/30' : 'text-indigo-100 hover:bg-indigo-600' }}" :class="!sidebarOpen ? 'justify-center' : ''">
                    ...
                    <div x-show="!sidebarOpen" class="absolute left-full top-1/2 -translate-y-1/2 ml-3 px-3 py-1.5 bg-slate-900 text-white text-xs font-semibold rounded-lg shadow-xl whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50">
                        Assistentes IA
                    </div>
                </div>
                ...
                <!-- EQUIPE & AGENDAS -->
                <div class="relative group">
                    <a href="/?view=equipe" class="flex items-center gap-3.5 px-3 py-2.5 rounded-xl transition font-semibold {{ $currentView === 'equipe' ? 'bg-indigo-900/90 text-white shadow-sm border border-indigo-500/30' : 'text-indigo-100 hover:bg-indigo-600' }}" :class="!sidebarOpen ? 'justify-center' : ''">
                ...
                <!-- CALENDÁRIO -->
                <div class="relative group">
                    <a href="/?view=agenda" class="flex items-center gap-3.5 px-3 py-2.5 rounded-xl transition font-semibold {{ $currentView === 'agenda' ? 'bg-indigo-900/90 text-white shadow-sm border border-indigo-500/30' : 'text-indigo-100 hover:bg-indigo-600' }}" :class="!sidebarOpen ? 'justify-center' : ''">
                ...
                <!-- SETTINGS -->
                <div class="relative group">
                    <a href="/?view=settings" class="flex items-center gap-3.5 px-3 py-2.5 rounded-xl transition font-semibold {{ $currentView === 'settings' ? 'bg-indigo-900/90 text-white shadow-sm border border-indigo-500/30' : 'text-indigo-100 hover:bg-indigo-600' }}" :class="!sidebarOpen ? 'justify-center' : ''">
        ...
        <!-- RODAPÉ DA SIDEBAR RENOMEADO PARA MULTIAGENTS V2.0 -->
        <div class="p-4 border-t border-indigo-600/80">
            <span x-show="sidebarOpen" class="text-[11px] bg-indigo-800 text-indigo-200 px-3 py-1 rounded-full font-medium border border-indigo-500 block text-center shadow-inner">Multiagents v2.0</span>
            <span x-show="!sidebarOpen" class="text-[10px] text-indigo-300 font-bold block text-center">v2.0</span>
        </div>
    </aside>
    ...
    <div class="flex-1 flex flex-col min-w-0">
        ...
        <div class="container mx-auto px-6 max-w-6xl mb-12">
                ...
                <div class="mt-8 space-y-6">
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                        <h2 class="text-lg font-bold text-gray-800 border-b border-gray-100 pb-3 mb-4 flex items-center justify-between">
                            <span>Gestão de Departamentos & Agentes</span>
                        </h2>
                        ...
                        <form action="/" method="POST" class="mb-8 bg-gray-50 p-4 rounded-xl border border-gray-200">
                            ...
                            <label class="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-2">Novo Departamento</label>
                            <div class="flex gap-3">
                                <input type="text" name="name" required placeholder="Ex: Suporte Técnico, Vendas..." class="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-xs focus:ring-2 focus:ring-indigo-500">
                                <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-4 py-2 rounded-lg text-xs transition">Criar Departamento</button>
                            </div>
                        </form>

                        <!-- LISTAGEM E ADIÇÃO DE AGENTES COM VALIDAÇÃO POR DEPARTAMENTO -->
                        <div class="space-y-6">
                            @forelse($departments as $dept)
                                <div class="bg-white border border-gray-200 rounded-xl p-5 shadow-sm space-y-4">
                                    ...
                                    <form action="/" method="POST" class="grid grid-cols-1 sm:grid-cols-3 gap-3 bg-slate-50 p-3 rounded-lg border border-slate-200">
                                        @csrf
                                        ...
                                        <div>
                                            ...
                                            <input type="text" name="name" required placeholder="Ex: Carlos Silva" class="w-full border border-slate-300 rounded-md p-2 text-xs">
                                        </div>
                                        <div>
                                            <label class="block text-[10px] font-bold text-slate-500 uppercase mb-1">E-mail Corporativo</label>
                                            <input type="email" name="email" required placeholder="carlos@empresa.com" class="w-full border border-slate-300 rounded-md p-2 text-xs">
                                        </div>
                                        <div class="flex items-end">
                                            <button type="submit" class="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2 rounded-md text-xs transition">
                                                Adicionar ao {{ $dept->name }}
                                            </button>
                                        </div>
                                    </form>
                                        ...
                                        <table class="w-full text-left text-xs border-collapse">
                                            ...
                                            <tbody class="divide-y divide-gray-50">
                                                @forelse($agents->where('department_id', $dept->id) as $agent)
                                                    <tr>
                                                        <td class="py-2.5 px-3 font-semibold text-gray-800">{{ $agent->name }}</td>
                                                        <td class="py-2.5 px-3 text-gray-600 font-mono">{{ $agent->email }}</td>
                                                    </tr>
                                                @empty
                                                    <tr><td colspan="2" class="py-4 text-center text-gray-400 italic">Nenhum agente cadastrado neste departamento.</td></tr>
                                                @endforelse
                                            </tbody>
                                        </table>
                                    </div>
                            ...
                            @empty
                                <div class="text-center py-8 text-gray-400">Nenhum departamento cadastrado ainda.</div>
                            @endforelse
                        </div>
                    </div>
                </div>
            ...
            @elseif($configuring)
                ...
                <div class="sticky top-0 z-40 bg-gray-50/90 backdrop-blur-md py-4 mb-6 border-b border-gray-200 flex flex-col md:flex-row items-center justify-between gap-4 shadow-sm mt-4">
