...
<body class="bg-gray-50 font-sans text-gray-900" x-data="{ 
    ...
    editAgentData: { id: '', name: '', email: '', phone: '' },
    selectedDept: null 
    ...
    <div class="container mx-auto px-4 max-w-6xl mt-8 mb-12">
                                ...
                                <li class="flex items-center justify-between group">
                                    <div class="flex items-center gap-2.5">
                                        ...
                                        <div class="leading-tight">
                                            ...
                                            <p class="text-[10px] text-gray-500">{{ $agent->email }} • {{ $agent->phone }}</p>
                                        </div>
                                    </div>
                                    <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition">
                                        <button type="button" @click="editAgentData = { id: '{{ $agent->id }}', name: '{{ addslashes($agent->name) }}', email: '{{ addslashes($agent->email) }}', phone: '{{ addslashes($agent->phone) }}' }; showEditAgentModal = true" class="text-gray-400 hover:text-indigo-600 transition p-1" title="Editar Agente">
                                        ...
                                        <form action="/?view=equipe" method="POST" onsubmit="return confirm('Remover {{ $agent->name }}?');">
                                            ...
                                            <button type="submit" class="text-red-300 hover:text-red-600 transition p-1" title="Excluir Agente"><svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>
                                        </form>
                                    </div>
                                </li>
                            @empty
                                        ...
                                        <div class="flex items-center justify-between bg-white border border-gray-200 rounded-lg p-1.5 shadow-sm pr-2 gap-3 group/agent w-auto">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <div class="leading-none">
                                                    ...
                                                    <span class="text-[9px] text-gray-400 block mt-0.5">{{ $agent->email }}</span>
                                                </div>
                                            </div>
                                            <div class="flex items-center gap-1 opacity-0 group-hover/agent:opacity-100 transition">
                                                <button type="button" @click="editAgentData = { id: '{{ $agent->id }}', name: '{{ addslashes($agent->name) }}', email: '{{ addslashes($agent->email) }}', phone: '{{ addslashes($agent->phone) }}' }; showEditAgentModal = true" class="text-gray-400 hover:text-indigo-600 bg-gray-50 hover:bg-indigo-50 rounded p-1 transition" title="Editar Agente">
                                                ...
                                                <form action="/?view=equipe" method="POST" onsubmit="return confirm('Remover {{ $agent->name }}?');">
                                                    ...
                                                    <input type="hidden" name="action" value="delete_agent">
                                                    <input type="hidden" name="agent_id" value="{{ $agent->id }}">
                                                    <button type="submit" class="text-gray-300 hover:text-red-500 bg-gray-50 hover:bg-red-50 rounded p-1 transition" title="Excluir Agente">
            ...
            <div @click.away="showAgentModal = false" class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
                ...
                <form action="/?view=equipe" method="POST">
                    ...
                    <div class="grid grid-cols-2 gap-3 mb-5">
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1">E-mail (Obrigatório)</label>
                            <input type="email" name="email" required placeholder="felipe@email..." class="w-full border border-gray-300 rounded-lg p-2 text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1">WhatsApp/Tel (Obrigatório)</label>
                            <input type="text" name="phone" required placeholder="11 99999-9999" class="w-full border border-gray-300 rounded-lg p-2 text-sm">
                        </div>
                    </div>
        ...
        <!-- MODAL EDITAR AGENTE -->
        <div x-show="showEditAgentModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4">
            <div @click.away="showEditAgentModal = false" class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
                <h3 class="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
                ...
                <form action="/?view=equipe" method="POST">
                    ...
                    <input type="hidden" name="agent_id" x-model="editAgentData.id">
                    
                    <label class="block text-xs font-bold text-gray-700 mb-1">Nome Completo</label>
                    <input type="text" name="name" x-model="editAgentData.name" required class="w-full border border-gray-300 rounded-lg p-2.5 text-sm mb-4">
                    
                    <div class="grid grid-cols-2 gap-3 mb-5">
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1">E-mail (Obrigatório)</label>
                            <input type="email" name="email" x-model="editAgentData.email" required class="w-full border border-gray-300 rounded-lg p-2 text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1">WhatsApp/Tel (Obrigatório)</label>
                            <input type="text" name="phone" x-model="editAgentData.phone" required class="w-full border border-gray-300 rounded-lg p-2 text-sm">
                        </div>
                    </div>
                    
                    <div class="flex gap-2 justify-end border-t border-gray-100 pt-4">
                        <button type="button" @click="showEditAgentModal = false" class="px-4 py-2 text-sm font-semibold text-gray-600 hover:bg-gray-100 rounded-lg">Cancelar</button>
                        <button type="submit" class="px-5 py-2 text-sm font-semibold bg-indigo-600 text-white hover:bg-indigo-700 rounded-lg shadow-sm">Salvar Alterações</button>
                    </div>
                </form>
            </div>
